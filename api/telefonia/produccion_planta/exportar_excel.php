...
$sheetRows .= '<row r="1">' . cStr('A1', 'LCM - La Cablera Marplatense', 1) . '</row>';
$sheetRows .= '<row r="2">' . cStr('A2', 'Producción Planta Externa', 2) . '</row>';

$detalle = 'Periodos: ' . implode(', ', $periodos)
    . ' | Zona: ' . ($zona ?: 'Todas')
    . ' | Tipo contratista: ' . $tipoContratista
    . ' | Tareas: ' . $tipo
    . ' | Contratista: ' . ($contratista ?: 'Todos');

$sheetRows .= '<row r="3">' . cStr('A3', $detalle, 3) . '</row>';
...
foreach ($rows as $r) {
    $ptr = round((float)$r['hb_ptrs']);
    $ocra = round((float)$r['hb_ocras']);
    $hb = round((float)$r['total_hb']);
    $venta = round((float)$r['venta']);
    $share = $totalHb > 0 ? round(((float)$r['total_hb'] / $totalHb), 4) : 0;
...
$sheetRows .= cNum('B' . $rowNum, $tPtr, 10);
$sheetRows .= cNum('C' . $rowNum, $tOcra, 10);
$sheetRows .= cNum('D' . $rowNum, $tHb, 10);
$sheetRows .= cNum('E' . $rowNum, $tVenta, 11);
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
          xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
    <sheets>
        <sheet name="Produccion Planta" sheetId="1" r:id="rId1"/>
    </sheets>
</workbook>';
